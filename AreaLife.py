...
# 동네생활 클래스 : 게시글 하나의 정보
class AreaLife:
    def __init__(self, life_id, user_id, title, content, status):
        self.life_id = life_id # 게시글 고유 ID
        self.user_id = user_id # 작성자
    ...
    def like(self, user):
        if user not in self.likes: # '공감하기'를 누른적이 없으면
            ...
            print(f'{user.name} 님이 공감하기를 눌렀습니다.')
        else:
            print(f'{user.name} 님은 이미 공감하기를 눌렀습니다.')

    # 사용자가 '공감하기'를 취소하는 함수
    def unlike(self, user):
        if user in self.likes: # '공감하기'를 누른 상태면
            self.likes.remove(user) # '공감하기' 집합에서 제거
            print(f'{user.name} 님이 공감하기를 취소했습니다.')
        else:
            print(f'{user.name} 님은 공감하기를 누르지 않았습니다.')
...
class Comment:
    def __init__(self, content, user_id):
        self.content = content # 댓글 본문
        self.user_id = user_id # 댓글 작성자(user)
